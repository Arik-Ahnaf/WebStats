...
export function Home({ snapshot, stale }: { snapshot: Snapshot; stale: boolean }) {
  ...
  return <main className="home-page">
    <section className="current-site" aria-label="Current site statistics">
      <SiteIcon hostname={hostname} large />
      <div className="current-details">
        <h1 title={hostname ?? undefined}>{hostname ?? 'Your time, here'}</h1>
    ...
    <section className="top-sites" aria-labelledby="top-sites-title">
      ...
      <div className="site-scroll" tabIndex={0} aria-label="Today's ranked sites">
        {sites.length ? <ol className="site-list">{sites.map((site, index) => <SiteRow key={site.hostname} site={site} rank={index + 1} />)}</ol>
